...
|---|---|---|---|
| 2026-08-17 | Ítem 8A(3) | OK | Tras retirar `static/index.html`/`app.js`/`styles.css` y los 3 `hamburg_mitte_*.geojson`: arranque OK, `/healthz` 200, `GET /` 200 sirviendo `index_original.html` (confirmado por contenido, no solo status). `/static/app_original.js` y `/static/styles_original.css` → 200 (el mount `/static` sigue sirviendo el frontend vivo sin cambios). `/static/index.html` y `/static/app.js` → 404 (correcto, ya borrados). Las tres capas (`/api/aeds`, `/api/boundary`, `/api/isochrones`) → 200. Apagado limpio, sin procesos huérfanos. |
| 2026-08-14 | Verificación visual en navegador (reportada por el usuario, no ejecutada por el agente) | Parcial — ver notas | **Cierra la limitación anotada en el registro de Fase 0** ("pasos 3 y 4 del checklist NO se ejecutaron literalmente — no hay navegador disponible en este entorno"). El usuario probó la app en local, en navegador real: las tres capas (AEDs, boundary, isócronas) cargan correctamente; modo Walk — click en el mapa devuelve ruta, se dibuja, el panel muestra los datos, correcto; modo Car — no encuentra nada (consistente con el hallazgo estructural ya conocido: la arista de acceso AED tiene `can_drive=False` siempre). **Hallazgo nuevo de UX, severidad relevante:** el fallo del modo Car es completamente silencioso. Verificado en `static/app_original.js`: cuando `POST /api/route` devuelve `results: []`, `renderPanel()` (línea 133: `if (!state.results.length) { showHint(); return; }`) llama a `showHint()` (línea 339), que simplemente vuelve a mostrar el div `#hint` con el mismo texto estático genérico que ya se veía ANTES del click ("Click on the map to find the nearest AED.", `index_original.html:67`) — no hay ningún mensaje que diga "sin ruta encontrada" ni distinga "no se ha hecho ningún click" de "se hizo un click y no hubo resultado". El usuario no tiene forma de saber qué pasó. Esto no es exclusivo de Car: cualquier click sin resultados (sobre agua, o a más de `MAX_SNAP_DISTANCE_M` de cualquier nodo) cae exactamente en la misma rama de código y falla igual de callado hoy. Ver análisis de mitigación en el mensaje de la Fase 4 (2026-08-14). |
